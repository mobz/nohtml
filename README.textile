...
jquery-nohtml has a very simple purpose
* create dom elements directly in javascript
* merge json and nohtml template easily
* bind event handlers directly within templates
* assign jquery data to elements within templates
* automatically escapes text (nohtml is invulnerable to "XSS":https://www.owasp.org/index.php/Cross-site_Scripting_(XSS) )

h2. Give me a simple example

Each nohtml template is simply a plain old javascript object. The format of object _looks_ like the html you want to produce
...
_The_ reason that noHtml is so powerful is that it is *all* javascript. Standard templating languages force you to do things like <% name %> or ${name} in the middle of a string of html, to get at javascript. Things that should be easy like looping, calling functions and accessing variables is _hard_.

This plug-in elevates templates to the status of first class constructors in jquery, and all jquery methods and plugins that accept HTML strings, also accept noHtml templates.
...
usually you use @text@ for strings, @child@ for any single element and @children@ for an array of elements
...
Note that the @map@ function is not natively available in some older browser (eg IE7), but can be implemented "very easily":https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/array/map#Compatibility
